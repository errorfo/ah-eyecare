...
@app.route("/tryon")
def tryon():
    from models import Product  # Import inside to avoid circular imports

    # 1. Get frames from database where admin enabled VR
    db_frames = [
        {'vr_image_url': product.vr_image_url}
        for product in Product.query.filter_by(vr_enabled=True).all()
    ]

    # 2. Default frames stored in static folder
    default_frames = [
        {'vr_image_url': url_for('static', filename='pics/glasses_PNG54292.png')},
        {'vr_image_url': url_for('static', filename='pics/metal-frame-geek-glasses-flat-symbol-icon-illustration-png.webp')},
        {'vr_image_url': url_for('static', filename='pics/glasses3.png')}
    ]

    # 3. Merge both lists
    all_frames = db_frames + default_frames

    # 4. Pass to template
    return render_template("tryon.html", frames=all_frames)


    # 3. Merge both — database frames first, then defaults
    # Convert DB objects to dict so they match default format
    db_frame_dicts = [{'vr_image_url': f.vr_image_url} for f in db_frames]

    all_frames = db_frame_dicts + default_frames

    return render_template("tryon.html", products=all_frames)
