...
REMOVE_BG_API_KEY = "728Defd6BTLYiEAo6JdyE6JG"  # Replace with your API key

def remove_bg_any_format(input_path, output_path):
    try:
        # 1. Try Remove.bg API
        ...
        if response.status_code == 200:
            ...
            print("Background removed using remove.bg API")
        else:
            raise Exception(f"Remove.bg failed: {response.status_code} {response.text}")

    except Exception as e:
        print(f"Falling back to local background removal: {e}")
        # 2. Local background removal (no internet needed)
        with open(input_path, 'rb') as inp:
            input_image = inp.read()
            output_image = remove(input_image)
            with open(output_path, 'wb') as out:
                out.write(output_image)
        print("Background removed locally with rembg")

# Usage:
# remove_bg_any_format("uploads/image.jpg", "uploads/image_no_bg.png")
...
def add_product():
    ...
    if vr_image_file and vr_image_file.filename:
        ...
        vr_image_file.save(original_path)

        if vr_enabled:
            # Output path with transparent background
            transparent_filename = f"vr_{os.path.splitext(filename)[0]}.png"
            transparent_path = os.path.join(app.config['UPLOAD_FOLDER'], transparent_filename)
            remove_bg_any_format(original_path, transparent_path)
            vr_image_url = f"/static/uploads/{transparent_filename}"
        else:
...
def tryon():
    ...
    },
    {
        ...
        'price': '',
        'vr_image_url': url_for('static', filename='pics/WhatsApp_Image_2025-08-14_at_19.53.21_57787560-removebg-preview.png'),
        'available': False
    },
    {
        'id': None,
        ...
        'price': '',
        'vr_image_url': url_for('static', filename='pics/WhatsApp_Image_2025-08-14_at_19.54.15_2efdaef2-removebg-preview.png'),
        'available': False
    },
    {
        'id': None,
        ...
        'price': '',
        'vr_image_url': url_for('static', filename='pics/WhatsApp_Image_2025-08-14_at_19.54.16_3569bf55-removebg-preview.png'),
        'available': False
    }
]
...
def add_review(product_id):
    ...
    review = Review(product_id=product.id, name=name, rating=rating, comment=comment)
